...
def get_rounds(number):
    ...
    next_three_rounds = [number, number + 1, number + 2]
    return next_three_rounds
...
def approx_average_is_average(hand):
    ...
    middle_hand = len(hand) // 2 
    if (hand[0] + hand[-1]) / 2 == average_hand:
        return True
    elif hand[middle_hand] == average_hand:
        return True
    return False
